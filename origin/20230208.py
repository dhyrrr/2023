...
class Fsec:
    def __init__(self, data_file_path, pptx_template_file_path, complex_protein_info_xlsx, writer, standard_curve_dict, peak_x_error, peak_prominence, Ex_Wavelength, save_dir,Free_GFP,ppt_file,image_file_path
                 ):
    ...
    def load_configuration():
        ...
        return data_file_path, pptx_template_file_path, complex_protein_info_xlsx, writer, a, b, peak_x_error, peak_prominence, Ex_Wavelength, save_dir,Free_GFP,ppt_file, image_file_path
...
//type -b "Hello LabTalk!";\n''')
        ...
        for slide in prs.slides:
            ...
            for shape in slide.shapes:
                ...
                if isinstance(shape, Picture):
                    with open(os.path.join(image_file_path, f'{index}.jpg'), 'wb') as f:
            ...
            for file in files:
                if re.search("jpg$", file):
                    ...
                    try:
                        ...
                        oldname = image_file_path + os.sep + file + '.jpg'
                        newname = image_file_path + os.sep + file_name + '.jpg'
            ...
            for i in code_list:
                if i.split("_")[1] == str(max(max_list)):
                    if i.split("_")[2] == str(0):
                        # print(i)
                        name = image_file_path + os.sep + i + '.jpg'
            # print(name)
    ...
    def generate_ppt(self):
        ...
        for page_index, column_name in enumerate(self.ppt_opju_name_list):
            ...
            # # height = Cm(15)
            # width = Cm(23)
            image_path_list=self.choose_image()
            #######打印图片路径
            # print(image_path_list)
            image_path=image_path_list[page_index]
            print(plasmidno+"_胶图路径："+image_path)

            ######插入图片
            pic = opju_pic1.insert_picture(image_path)
            # 图片置于文本框下方
...
def main():
    ...
    # 导入配置文件
    data_file_path, pptx_template_file_path, complex_protein_info_xlsx, writer, a, b, peak_x_error, peak_prominence, Ex_Wavelength, save_dir,Free_GFP,ppt_file,image_file_path = Fsec.load_configuration()
    ...
    ######加入胶图的pptx文件
    SingleSearchparser.add_argument("ppt_file", metavar='小试PPT',
                                    widget='FileChooser',
    ...
    if args.command == "FSEC数据分析":
        ...
        # 生成结果保存目录
        result_file_name_list = ["ogs", "opju", "ppt"]
        ...
        # 实例化
        fsec_obj = Fsec(data_file_path, pptx_template_file_path, complex_protein_info_xlsx, writer, standard_curve_dict, peak_x_error, peak_prominence, Ex_Wavelength, save_dir,Free_GFP,ppt_file,image_file_path)
